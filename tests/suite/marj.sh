...
check "a modified tracked file (leading-space status) keeps its full path" n "$EX import {writeFileSync} from 'node:fs'; const s=repoSnapshot('$D'); writeFileSync('$D/b.txt','b2'); const d=repoDelta(s); if(JSON.stringify(d.files)!==JSON.stringify(['b.txt'])) throw JSON.stringify(d)"
check "mergeDelta normalizes vendor absolute paths against cwd" n "$EX const r=mergeDelta({files_changed:['$D/b.txt','$D/x/y.js']},{files:['b.txt'],commits:[]},'$D'); if(JSON.stringify(r.files_changed)!==JSON.stringify(['b.txt','x/y.js'])) throw JSON.stringify(r)"
...
check "…Codex was granted the sidecar board dir (--add-dir) so the facade can write it" bash -c "grep -q -- '--add-dir $GBOARD' '$CODEX_STUB_LOG'"
