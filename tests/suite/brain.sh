...
echo "brain — project-scoped token (least privilege, server-enforced):"
check "autodev brain token mints a scoped token and stores it in the runtime dir (0600)" bash -c "cd '$R' && BRAIN_TOKEN=stub-token node '$CLI' brain token | grep -q 'scoped token for prj_' && f=\$(ls '$AUTODEV_HOME'/state/projects/*/runtime/brain.token | head -1) && [ \"\$(stat -c %a \"\$f\" 2>/dev/null || stat -f %Lp \"\$f\")\" = 600 ] && grep -q '^brn_stub_' \"\$f\""
check "…the request named exactly this project + non-admin permissions" bash -c "grep '/v1/tokens' '$BRAIN_STUB_LOG' | tail -1 | jq -e '.body.projects|length==1' && grep '/v1/tokens' '$BRAIN_STUB_LOG' | tail -1 | jq -e '.body.permissions|index(\"admin\")==null'"
check "from now on this project uses the scoped token"    bash -c "cd '$R' && BRAIN_TOKEN= node '$CLI' brain status | grep -q 'token from .*runtime/brain.token'"
check "…and the stub saw it on the wire"                 bash -c "tail -1 '$BRAIN_STUB_LOG' | jq -e '.auth|test(\"brn_stub_\")'"
check "the runtime dir is ignored by the state repo (never committed)" bash -c "cd '$AUTODEV_HOME/state' && git check-ignore -q projects/x/runtime/brain.token"
check "a second call is a no-op"                          bash -c "cd '$R' && BRAIN_TOKEN= node '$CLI' brain token | grep -q 'already using a project-scoped token'"
check "env still wins when set (operator override)"       bash -c "cd '$R' && BRAIN_TOKEN=stub-token node '$CLI' brain status | grep -q 'token from env'"
rm -f "$AUTODEV_HOME"/state/projects/*/runtime/brain.token   # leave the fixture as it was for the degraded checks below
