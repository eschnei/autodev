...
check "autodev brain token mints a scoped token and stores it in the runtime dir (0600)" bash -c "cd '$R' && BRAIN_TOKEN=stub-token node '$CLI' brain token | grep -q 'scoped token for prj_' && f=\$(ls '$AUTODEV_HOME'/state/projects/*/runtime/brain.token | head -1) && [ \"\$(stat -f %Lp \"\$f\")\" = 600 ] && grep -q '^brn_stub_' \"\$f\""
