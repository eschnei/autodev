...
check "…the Brain project id is stored in the SIDECAR project.json, not the repo" bash -c "id=\$(grep 'Project:' <<<\"\$0\" | grep -oE 'prj_[0-9A-Z]{26}' | head -1); jq -e '.brain.enabled==true and (.brain.project_id|startswith(\"prj_\"))' \"$AUTODEV_HOME/projects/\$id/project.json\" && [ -z \"\$(git -C '$R' status --porcelain)\" ]" "$OUT"
: > "$BRAIN_STUB_LOG"
OUT=$(cd "$R" && node "$CLI" status 2>&1)
...
check "…provenance event in the sidecar: bundle id + memory revisions" bash -c "id=\$(cd '$R' && node '$CLI' status 2>/dev/null | grep -oE 'prj_[0-9A-Z]{26}' | head -1); cat \"$AUTODEV_HOME/projects/\$id/events/\"*.jsonl | jq -e 'select(.type==\"brain.context.supplied\") | (.context_bundle_id|startswith(\"ctx_\")) and (.memories|length)==2 and (.memories[0].revision|type)==\"number\"' | grep -q true"
check "…handoff event recorded"                         bash -c "id=\$(cd '$R' && node '$CLI' status 2>/dev/null | grep -oE 'prj_[0-9A-Z]{26}' | head -1); cat \"$AUTODEV_HOME/projects/\$id/events/\"*.jsonl | grep -q '\"type\":\"brain.handoff.recorded\"'"
