...
the transitions the model performs (PRD/breakdown authoring, development, QA
angle spawning, merge-verify mechanics), the one-feature lock, repro-first bug
flow, intake interviews, backlog drain, and the concierge itself. These are prose
executed by the model; the harness deliberately never invokes one. Since package
2 (§6) the gate decisions, story selection, and next-action are core logic and
ARE tested.
...
| 2 | **Gate 1 — plan approval** | `src/core/workflow/gates.mjs` (v3, local board) · `commands/loop.md` step 1, `reference/devloop.md §0` (plugin path) | Nothing is built until a human approves the PRD. Approval is an explicit human act: `autodev approve <id>` records it (audit comment + move to Breakdown + sidecar event) and hands the model one bounded breakdown job; in the plugin path, an in-session "approved", an authorized `approve` comment, or moving the card. The engine **never** crosses the gate itself. | **yes** (local board) / no (API trackers, plugin path) | done M8 (local) · API trackers when their core wrapper lands | `tests/suite/workflow.sh` |
| 3 | **PRD authoring** | `reference/prd.md` | A brief becomes a PRD with testable acceptance criteria, stops at `PRD Review (H)`. `planning.engine: agency` (the product-manager persona) is the default; BrainGrid is an optional adapter that falls back to agency. BYO-PRD skips re-authoring and produces one approval package. | partial (planning-engine selection + fallback derivation mechanical; authoring prose) | done M7 (default) · authoring stays a model job | `tests/suite/config.sh` |
...
| 5 | **Story selection** | `src/core/workflow/state.mjs` (v3) · `reference/devloop.md §1–2` (plugin path) | Per lane (≤`execution.max_lanes`): oldest `Ready for AI Dev` story carrying `ai-eligible` + this instance's label, whose blockers are done and whose touched files don't overlap in-flight work. None → wait / await human / idle. The one-feature lock and its epic-level logging remain prose. | **partial** (selection + next-action mechanical; feature lock prose) | M8 (done: selection) · lock in a later M8 slice | `tests/suite/workflow.sh` |
...
| 9 | **Gate 2 — result review** | `src/core/workflow/gates.mjs` (v3, local board) · `reference/devloop.md §7`, `manual ▸ Delivery mode` | `per_story`: QA reports + manual script posted, `ready_for_human_review`; `autodev approve <id>` records the decision, hands the model the bounded squash-merge + merge-verify job, and **verifies the card reached `done`** (a merge the model only claims is reported as not done). `autodev reject <id> <reason>` returns it to AI Development with the reason. `per_feature`: acceptance approval hands a ship job. Nothing reaches the default branch without a human. | **yes** (decision + verification, local board) / no (the merge itself is a model job) | done M8 (local) · merge mechanics move to git core later | `tests/suite/workflow.sh` |
...
| 16 | **Own lane on a shared board** | `src/core/tracker.mjs ▸ ownIssues`, `src/core/workflow/state.mjs` · `manual ▸ principle 10` | Every read and write is scoped to issues carrying `tracker.instance_label`. Foreign tickets do not exist to core's state (counted as `foreign`, never selected). First connect to a populated board asks once (prose). | **yes** (core reads/selection) / no (the ask) | done M4/M8 | `tests/suite/core.sh`, `tests/suite/workflow.sh` |
| 17 | **Every action leaves a board trail** | `manual ▸ principle 9`, `devloop §9` · `src/core/events.mjs` | Status = where; comments = what + why. A floor in every `execution.logging` mode. Moves carry `--note`. Gate decisions also append an `evt_` event to the sidecar history. Errors are posted on the affected story, never swallowed. One metrics line per tick. | partial (tracker history + gate events mechanical; per-tick narration prose) | M8 slices → M13 (events mirror to Brain) | `tests/suite/tracker.sh`, `tests/suite/workflow.sh` |
...
| 21 | **Concierge + session modes** | `hooks/session-signal.sh`, `manual ▸ Concierge` · `src/cli/main.mjs` | `concierge` injects identity + manual + conventions; `signal` one line; `silent` nothing; unconfigured repos are never hijacked. The CLI shows the same status snapshot and an "Awaiting you" line from deterministic state. Plain-English routing is the model's. The plugin stays a first-class install path (operator decision 2026-09-05), not a migration shim. | partial (hook modes + CLI status) | done M2 | `tests/suite/cli.sh` |
...
| 23 | **Config compatibility** | `src/core/config/schema.mjs` (v3) · `scripts/lib/config.*`, `scripts/upgrade-config.sh`, `reference/deployment.example.json` | Existing `.autodev/deployment.json` + `deployment.local.json` keep loading unchanged; a legacy `braingrid.enabled=true` still means BrainGrid; a pre-local config still means Linear. Upgrades add schema defaults under operator values and never delete or reorder operator keys. The example is tested leaf-for-leaf against the schema. | **yes** | done M5 · M5A (sidecar registry reads legacy config through the same loader — done in M2) | `smoke`, `tests/suite/config.sh` |
...
## 6. Package 2 (M4–M8) — what moved from contract to mechanical

| Milestone | Now mechanical | Where |
|---|---|---|
| M4 | Core tracker reads + facade writes, git helpers + worktree strategy, config loader used by tick/CLI/registry | `tests/suite/core.sh`, `cli.sh` |
| M5 | One config schema: example parity, validation, legacy normalization, upgrade migrations, doctor gate | `tests/suite/config.sh` |
...
| Milestone | Where | Mechanical |
|---|---|---|
| M15 | `src/executors/codex/index.mjs`: CodexExecutor on the same Job contract through the user's `codex exec` (subscription-native; `-c approval_policy=never` (codex ≥0.150 dropped `-a`), workspace-write sandbox, network only when the allowlist grants push/gh; JSONL events normalized to files_changed / tests_failed / summary; rate-limit, failed, unavailable). Codex has no per-command allowlist, so the hard rules ride in the prompt and `capabilities().tool_allowlist=false` tells core the guard is prompt-level; `reference/*.md` mentions are inlined so tasks are self-contained. `autodev executor codex` persists the choice; the tick honors it. All `codex` invocation lives in the adapter (tested). | `tests/suite/core.sh`, `cli.sh` |
...
| Marj 0–4, 6 | `src/control/api.mjs` (Control API: the single validated, audited action surface; model-neutral), `src/controller/controller.mjs` (controller contract + structured intent + constraint enforcement), `src/controller/claude/index.mjs` (ClaudeCodeController, tool-less `claude -p`, JSON intent), `src/control/mcp.mjs` (`autodev mcp`: the same surface over MCP stdio, contract as server instructions), CLI `Marj >` with deterministic bypass and `autodev >` fail-safe, `controller` config section, `autodev projects` cross-project attention, `project.paused` honored by the tick. See `docs/v3/marj.md`. | `tests/suite/marj.sh` |
| Hardening (M8A, M12A) | `src/core/workspace.mjs` isolated story lanes (worktrees under the sidecar runtime dir; `list_lanes` / `release_lane`), `src/core/contamination.mjs` contamination guard (every job + verification + status; never deletes), executors report `files_changed` from the git delta, CLI mutations are Control API calls, `package.json` `files` allowlist. See `docs/v3/hardening.md`. | `tests/suite/marj.sh` (hardening section) |
